...
// CTA spesifik per window (SCAN MY BREAKFAST/LUNCH/DINNER) sesuai template resmi.
const MEAL_UI = {
  id: {
    howItWorks: "Cara kerja",
    ctaByWin: { breakfast: "SCAN SARAPANKU", lunch: "SCAN MAKAN SIANGKU", dinner: "SCAN MAKAN MALAMKU" },
    steps: [["Snap", "Foto piringmu"], ["Scan", "Terdeteksi otomatis"], ["Done", "Asupan tercatat"]],
    micro: "Gratis di aplikasi 20FIT &nbsp;&bull;&nbsp; Kurang dari semenit",
    heroAlt: "20FIT — Nutrisi lebih pintar dimulai di sini. Scan makananmu untuk cek kalori & nutrisi.",
    teamLine: "Tim 20FIT",
    footerTag: "Seluruh cerita kesehatanmu, dalam satu tempat.",
    footerWhy: "Kamu menerima email ini karena kamu member 20FIT.",
    footerUnsub: "Berhenti berlangganan",
    footerCompany: "PT Kredo AUM, Jakarta, Indonesia",
  },
  en: {
    howItWorks: "How it works",
    ctaByWin: { breakfast: "SCAN MY BREAKFAST", lunch: "SCAN MY LUNCH", dinner: "SCAN MY DINNER" },
    steps: [["Snap", "Photo your plate"], ["Scan", "Auto-detected"], ["Done", "Intake logged"]],
    micro: "Free in the 20FIT app &nbsp;&bull;&nbsp; Takes less than a minute",
    heroAlt: "20FIT — Smarter Nutrition Starts Here. Scan your meal to check calories & nutrition.",
    teamLine: "Team 20FIT",
    footerTag: "Your whole health story, in one place.",
    footerWhy: "You're receiving this email because you're a 20FIT member.",
    footerUnsub: "Unsubscribe",
    footerCompany: "PT Kredo AUM, Jakarta, Indonesia",
  },
};
// Label pill per window & bahasa (title-case; dot dirender di builder).
const WINDOW_PILL = {
...
// Struktur mengikuti template email resmi 20FIT (hero Anton + panel 3-langkah pink +
// tombol CTA per meal + footer perusahaan). Slot yang berubah antar varian:
// subject/headline/body/signoff + CTA/pill per window. Layout/warna TETAP.
function mealReminderHtml(win, v, scanUrl, unsubscribeUrl, lang) {
  const L = lang === "en" ? "en" : "id";
  const ui = MEAL_UI[L];
  const pill = (WINDOW_PILL[L] && WINDOW_PILL[L][win]) || WINDOW_PILL.en.lunch;
  const cta = (ui.ctaByWin && ui.ctaByWin[win]) || ui.ctaByWin.lunch;
  const preheader = v.body;
  const step = (n, t, s) =>
    "<td width='33%' valign='top' align='center'>" +
    "<table role='presentation' cellpadding='0' cellspacing='0' align='center'><tr>" +
    "<td align='center' width='46' height='46' style='width:46px;height:46px;background:" + RED + ";border-radius:23px;font-family:Arial,Helvetica,sans-serif;font-size:19px;font-weight:bold;color:#ffffff;text-align:center;line-height:46px'>" + n + "</td>" +
    "</tr></table>" +
    "<div style='font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:bold;color:" + INK + ";padding-top:12px'>" + t + "</div>" +
    "<div style='font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#7A7A7A;line-height:16px;padding-top:3px'>" + s + "</div></td>";
  ...
  return (
    ...
    "<style>@media screen and (max-width:600px){.container{width:100% !important}.px{padding-left:24px !important;padding-right:24px !important}.h1{font-size:32px !important;line-height:36px !important}}</style>" +
    "</head><body style='margin:0;padding:0;background-color:" + PAGE_BG + "'>" +
    ...
    "<table role='presentation' width='100%' cellpadding='0' cellspacing='0' style='background-color:" + PAGE_BG + "'><tr><td align='center' style='padding:28px 12px'>" +
    "<table role='presentation' class='container' width='600' cellpadding='0' cellspacing='0' style='width:600px;max-width:600px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 6px 24px rgba(0,0,0,0.06)'>" +
    // hero clickable
    "<tr><td style='padding:0;font-size:0;line-height:0'><a href='" + scanUrl + "' target='_blank' style='text-decoration:none'><img src='" + HERO_IMG + "' width='600' alt='" + ui.heroAlt + "' style='display:block;width:100%;max-width:600px;height:auto;border:0'></a></td></tr>" +
    // time pill (dot + label)
    "<tr><td class='px' align='left' style='padding:28px 40px 0 40px'><table role='presentation' cellpadding='0' cellspacing='0'><tr>" +
    "<td style='background-color:" + PILL_BG + ";border-radius:20px;padding:7px 16px;font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:bold;letter-spacing:1.5px;color:" + RED + ";text-transform:uppercase'>&#9679;&nbsp; " + pill + "</td>" +
    "</tr></table></td></tr>" +
    // headline (Anton, fallback Arial Black)
    "<tr><td class='px' align='left' style='padding:14px 40px 0 40px'>" +
    "<h1 class='h1' style=\"margin:0;font-family:'Anton','Arial Black',Arial,sans-serif;font-size:42px;line-height:46px;color:" + INK + ";font-weight:400;text-transform:uppercase;letter-spacing:0.5px\">" + v.headline + "</h1></td></tr>" +
    // body
    "<tr><td class='px' align='left' style='padding:16px 40px 0 40px'><p style='margin:0;font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:26px;color:#4B4B4B'>" + v.body + "</p></td></tr>" +
    // panel 3 langkah (pink)
    "<tr><td class='px' align='left' style='padding:24px 40px 0 40px'>" +
    "<p style='margin:0 0 12px 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:bold;letter-spacing:1.5px;color:" + INK + ";text-transform:uppercase'>" + ui.howItWorks + "</p>" +
    "<table role='presentation' width='100%' cellpadding='0' cellspacing='0' style='background-color:" + PILL_BG + ";border-radius:14px'><tr><td style='padding:24px 12px'>" +
    "<table role='presentation' width='100%' cellpadding='0' cellspacing='0'><tr>" +
    step("1", ui.steps[0][0], ui.steps[0][1]) + step("2", ui.steps[1][0], ui.steps[1][1]) + step("3", ui.steps[2][0], ui.steps[2][1]) +
    "</tr></table></td></tr></table></td></tr>" +
    // CTA (centered) + VML fallback Outlook
    "<tr><td class='px' align='center' style='padding:28px 40px 0 40px'>" +
    "<!--[if mso]><v:roundrect xmlns:v='urn:schemas-microsoft-com:vml' xmlns:w='urn:schemas-microsoft-com:office:word' href='" + scanUrl + "' style='height:50px;v-text-anchor:middle;width:250px;' arcsize='20%' strokecolor='" + RED + "' fillcolor='" + RED + "'><w:anchorlock/><center style='color:#ffffff;font-family:Arial,sans-serif;font-size:15px;font-weight:bold;letter-spacing:0.5px;'>" + cta + "</center></v:roundrect><![endif]-->" +
    "<!--[if !mso]><!-- --><a href='" + scanUrl + "' target='_blank' style='display:inline-block;background-color:" + RED + ";padding:15px 34px;font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:bold;letter-spacing:0.5px;color:#ffffff;text-decoration:none;border-radius:10px;box-shadow:0 3px 10px rgba(214,40,40,0.30)'>" + cta + "</a><!--<![endif]-->" +
    "</td></tr>" +
    // microcopy (centered)
    "<tr><td class='px' align='center' style='padding:12px 40px 0 40px'><p style='margin:0;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#9A9A9A'>" + ui.micro + "</p></td></tr>" +
    // signoff (centered) + Team line
    "<tr><td class='px' align='center' style='padding:22px 40px 30px 40px'><p style='margin:0;font-family:Arial,Helvetica,sans-serif;font-size:16px;line-height:26px;color:#4B4B4B'>" + v.signoff + "<br><strong style='color:" + INK + "'>" + ui.teamLine + "</strong></p></td></tr>" +
    // divider
    "<tr><td style='padding:0 40px'><div style='border-top:1px solid #EAE8E5;font-size:0;line-height:0'>&nbsp;</div></td></tr>" +
    // footer
    "<tr><td class='px' align='center' style='padding:22px 40px 30px 40px'>" +
    "<img src='" + LOGO_IMG + "' width='120' alt='20FIT' style='display:block;margin:0 auto 6px auto;width:120px;max-width:120px;height:auto;border:0'>" +
    "<p style='margin:0 0 8px 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:18px;color:#9A9A9A'>" + ui.footerTag + "</p>" +
    "<p style='margin:0 0 6px 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:18px;color:#9A9A9A'>" + ui.footerWhy + "</p>" +
    "<p style='margin:0;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:18px;color:#9A9A9A'><a href='" + unsubscribeUrl + "' style='color:#9A9A9A;text-decoration:underline'>" + ui.footerUnsub + "</a> &nbsp;&bull;&nbsp; " + ui.footerCompany + "</p>" +
