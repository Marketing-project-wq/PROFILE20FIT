...
async function runMonitor(ctx) {
  ...
  for (const camp of [{ id: "meal_reminder", channel: "meal_reminder" }, { id: "onboarding_no_scan", channel: "marketing", campaignId: "onboarding_no_scan" }]) {
    ...
    // Butuh volume minimum biar rate tak liar (mis. 1 dari 2). Ambil >=50 kirim.
    let breach = null;
    if (m.dispatched >= 50) {
      if (m.complaint_rate > 0.1) breach = "complaint " + m.complaint_rate + "%";
      else if (m.bounce_rate > 2) breach = "bounce " + m.bounce_rate + "%";
      else if (m.unsub_rate > 0.5) breach = "unsub " + m.unsub_rate + "%";
    }
    if (breach) {
      try {
