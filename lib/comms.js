...
async function canSend(o) {
  ...
  if (bucket === "marketing") {
    if ((await countRecent(userId, h24, "marketing")) >= 1) return { ok: false, reason: "cap_marketing_daily" };
    if ((await countRecent(userId, d7, "marketing")) >= 3) return { ok: false, reason: "cap_marketing_weekly" };
  } else if (bucket === "meal_reminder") {
    if ((await countRecent(userId, h24, "meal_reminder")) >= 3) return { ok: false, reason: "cap_meal_daily" };
  }

  // 4) jangan ada email non-transaksional lain dalam 60 menit terakhir (anti-tumpuk)
  if ((await countRecent(userId, h1, null)) >= 1) return { ok: false, reason: "cooldown_60m" };
