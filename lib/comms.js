...
// Set bahasa email pilihan user. lang: 'id' | 'en'. (Consent dihapus — semua email
// dikirim langsung; opt-out lewat unsubscribe/suppression, bukan opt-in.)
async function setLangByUser(userId, lang) {
  if (!_admin || !userId) return null;
  if (lang !== "id" && lang !== "en") return getPrefsByUser(userId);
  await ensurePrefs(userId);
  const { data } = await _admin
    .from("my20fit_user_comm_prefs")
    .update({ lang: lang, updated_at: new Date().toISOString() })
    .eq("user_id", userId)
    .select("*")
    .single();
  return data || null;
...
async function canSend(o) {
  ...
  // 1) suppression / hard bounce / unsubscribe (opt-out — consent opt-in dihapus)
