...
async function renderTemplate(admin, campaigns, templateId, baseUrl, unsubscribeUrl) {
  ...
  if (templateId === "onboarding_welcome") {
    // Blast admin default bahasa ID. Deep link kartu fitur ke halaman masing-masing.
    const urls = { scanUrl: scanUrl, prefsUrl: unsubscribeUrl, loginUrl: baseUrl,
      dashboardUrl: baseUrl + "/dashboard.html", medicalUrl: baseUrl + "/medical.html", progressUrl: baseUrl + "/progress.html" };
    return campaigns.onboardingWelcomeHtml("id", urls);
  }
...
async function createSend(ctx, opts) {
  ...
  const { eligible, breakdown } = await segments.applyEligibility(admin, matched);
...
async function processBatch(ctx) {
  ...
  for (const r of batch) {
    // CEK ULANG suppression (bisa berubah sejak dibekukan — unsubscribe di tengah).
    const suppressed = await comms.isSuppressed(r.email);
    if (suppressed) {
      await admin.from("my20fit_email_send_recipients").update({ status: "skipped", error_message: "suppressed" }).eq("id", r.id);
      skipped++; continue;
    }
...
async function runAutomations(ctx) {
  ...
  for (const a of autos || []) {
    ...
    const { eligible } = await segments.applyEligibility(admin, fresh);
