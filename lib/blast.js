...
function renderTemplate(campaigns, templateId, baseUrl, unsubscribeUrl) {
  ...
  const variant = campaigns.MEAL_VARIANTS[win] ? campaigns.MEAL_VARIANTS[win][0] : null;
  if (!variant) return null;
  return campaigns.mealReminderHtml(win, variant, scanUrl, unsubscribeUrl);
}
...
async function sendTest(ctx, sendId, toEmail) {
  ...
  const html = renderTemplate(campaigns, s.template_id, baseUrl, unsub);
...
module.exports = { TEMPLATES, TEMPLATE_IDS, AUTO_ABORT, createSend, sendTest, confirmSend, setStatus, processBatch, runAutomations };
