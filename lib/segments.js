...
// Balikan { eligible, excluded, breakdown }. Consent opt-in dihapus; kelayakan =
// tidak di suppression/unsubscribe, tidak di campaign lain, tidak baru dikirimi 24 jam.
async function applyEligibility(admin, matched) {
  ...
  // Ambil sinyal kelayakan secara batch. (Consent opt-in dihapus — semua user layak
  // kecuali suppression/unsubscribe, sedang di campaign lain, atau baru dikirimi 24 jam.)
  const [{ data: supp }, { data: enroll }, { data: recent }] = await Promise.all([
  ...
  const breakdown = { suppressed_unsubscribe: 0, suppressed_bounce: 0, suppressed_complaint: 0, suppressed_dormant: 0, suppressed_other: 0, in_other_campaign: 0, recent_24h: 0 };
  const eligible = [];
  const excluded = [];
...
async function previewSegment(admin, presetId) {
  ...
  const { eligible, breakdown } = await applyEligibility(admin, matched);
