...
> **Pembaruan dokumen terakhir:** 2026-09-16 · **Commit staging:** `7fc4998` · **Production:** `3d3db2c`
...
**Member (perlu login):** `/dashboard` (home 6-tile), `/calories`, `/progress`, `/profile`, `/medical`, `/diet`, `/recipe` (browse resep + kontribusi), `/classes` (Book Class, toggle Arena/Gym; `?venue=clinic`=Book Recovery), `/membership` (carousel — **data belum tersambung**), `/event` (**Ticket Wallet**: tab "Tiket Saya" + "Upcoming"; widget bersama `js/ticket-wallet.js`, dipakai juga di `/dashboard`), `/payment/pending|success|failed`.
