...
- **Tiket user tidak muncul — akar masalahnya DI LUAR repo ini (PR #417).** Tiket **tidak disimpan di Supabase kita**: sapuan `pg_stat_user_tables` menunjukkan tak ada tabel yang menerima pembelian tiket baru, dan `my20fit_orders` berisi **nol** `kind='ticket'`. Tiket hidup di **ticket.20fit.id**, dibaca lewat edge function `ticket-embed`.
  ...
  - **Jalur OTP penerbit (PR #419) — TERBUKTI BEKERJA.** `/otp/verify` benar-benar menerbitkan `userToken`: baris nyata di `my20fit_ticket_tokens` (dibuat 2026-09-08 14:04:32, token 248 karakter). Pertanyaan "apakah OTP menolong pembeli tamu" sudah terjawab: ya.
  - **⚠️ TEMUAN TERPENTING — token penerbit hanya hidup 900 detik (15 menit).** Baris yang sama: `expires_at` = 14:19:32, tepat 900 detik setelah dibuat. Dulu `getTicketToken` membuang token begitu jam kita lewat, jadi pembeli tamu diminta verifikasi ulang **setiap 15 menit** — itu yang terasa seperti "gate permanen", bukan gate di kode kita. Sejak PR #421 token tidak lagi dibuang berdasarkan jam lokal; kolom `expires_at` kini catatan diagnostik saja.
    **TERVERIFIKASI 2026-09-09 — penerbit BENAR-BENAR menegakkan 900 detik.** Token nyata yang terbit 2026-09-08 14:04 dikirim ulang ke `/me/tickets` ~16 jam kemudian dan dibalas **`401 {"error":"user_unauthorized","message":"Missing/invalid X-Embed-User-Token"}`**. (Diuji lewat `pg_net` dari dalam Postgres supaya nilai tokennya tidak pernah keluar dari database.)
    **Konsekuensinya: perubahan token di PR #421 TIDAK menyelesaikan masalah ini** — ia hanya memindahkan penolakan dari jam kita ke penerbit; user tetap kena imbasnya.
    **Yang bisa & tidak bisa diperbaiki dari sisi kita:**
    - Pemilik **akun** ticket.20fit.id → sudah aman: `/api/tickets/mine` jatuh ke `/partner/user-token` dengan email sesi, jadi token mati di-mint ulang **otomatis tanpa user sadar**. Akses mereka tidak pernah putus.
    - Pembeli **tamu** → **tidak ada jalur diam-diam**. Satu-satunya mint adalah OTP, dan OTP menuntut kode dari inbox user. Tidak ada endpoint refresh di API penerbit.
    **TANYA PEMILIK ticket.20fit.id:** minta TTL lebih panjang atau refresh token (sudah masuk `docs/TICKET-API-REQUEST.md`). Alternatif tanpa perubahan API: pembeli tamu mendaftar akun di ticket.20fit.id dengan email yang sama → pindah ke kasus pertama.
  - **Gate konfirmasi: TIDAK ADA di kode kita** (diselidiki 2026-09-09). Auto-resolve sudah berjalan tanpa aksi user sejak awal — `/api/tickets/mine` mengambil email dari `my20fit_profile` by `auth_user_id` lalu menukarnya ke token. Tombol verifikasi hanya muncul saat hasil = 0 (`js/ticket-wallet.js`, cabang `!TICKETS.length`). Tidak ada modal, route guard, checkbox, atau flag `isVerified`/`claimed` di mana pun.
  - **PR #421 (di `staging`) — QR langsung tampil.** `/api/tickets/mine` kini mengambil QR **semua tiket paralel** (`attachQrs`) dan menyertakannya di respons yang sama → **N+1 hilang**, QR tak lagi di balik tombol. Ditambah `event_qty` (dihitung dari pengelompokan, skema penerbit flat 1 baris = 1 tiket) dan `buyer` (dipetakan defensif; null kalau penerbit tak mengirimnya). **BELUM TERVERIFIKASI:** apakah penerbit memang mengirim nama pemesan — cek lewat `/api/tickets/mine?debug=1` sebagai superadmin.
  - **Production masih tertinggal:** `main` belum memuat PR #419/#421 — di sana widget hanya menampilkan pesan sebab, tanpa tombol verifikasi dan tanpa QR inline.
