...
- **Tiket user tidak muncul — akar masalahnya DI LUAR repo ini (PR #417).** Tiket **tidak disimpan di Supabase kita**: sapuan `pg_stat_user_tables` menunjukkan tak ada tabel yang menerima pembelian tiket baru, dan `my20fit_orders` berisi **nol** `kind='ticket'`. Tiket hidup di **ticket.20fit.id**, dibaca lewat edge function `ticket-embed`.
  ...
  - **⚠️ TEMUAN TERPENTING — token penerbit hanya hidup 900 detik (15 menit).** Baris yang sama: `expires_at` = 14:19:32, tepat 900 detik setelah dibuat. Dulu `getTicketToken` membuang token begitu jam kita lewat, jadi pembeli tamu diminta verifikasi ulang **setiap 15 menit** — itu yang terasa seperti "gate permanen", bukan gate di kode kita. Sejak PR #421 token tidak lagi dibuang berdasarkan jam lokal; penerbit yang berhak menolak (401 → jatuh ke jalur berikutnya). Kolom `expires_at` tetap ditulis sebagai catatan diagnostik. **BELUM TERVERIFIKASI:** apakah token >15 menit benar-benar ditolak penerbit — hanya bisa dibuktikan dengan menunggu >15 menit lalu memuat ulang di staging.
