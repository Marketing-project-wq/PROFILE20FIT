...
`auth.js` (Auth: Supabase client, FITCO login, OTP, profile CRUD, scan quota, routing; `Auth.ready` is a Promise, `Auth.token()`), `i18n.js` (EN/ID, `window.L`, lang toggle, `localStorage.lang`), `nav.js` (sidebar + bottom-nav + Scan FAB), `recipes.js` (recipe dataset — **server-side only**, not loaded in browser; used by `loadMenuCatalog()` for `/api/menu/catalog` & `/api/menu/recommend`), `recipe-photos.js` (food-photo resolver: `window.RecipePhotos` — resolveImg, applyThumb, applyThumbLazy; ~2KB, loaded by `recipe.html` & `calories.html`), `nutrition.js` (healthy-food composer), `fasting.js` (IF styles/timer), `deals.js` (scan-credit top-up flow), `tour.js` (per-page walkthroughs), `cycle.js` (menstrual phase tips), `achievements.js` (badges), `orders.js` (per-device order history — **not authoritative**), `pw-toggle.js` (password show/hide), `meta-pixel.js` (Pixel + CAPI dedup).
